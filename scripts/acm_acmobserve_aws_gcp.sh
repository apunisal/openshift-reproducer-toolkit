...
EOF
    ...
    if ! resource_exists "secret" "thanos-object-storage" "$OBS_NAMESPACE"; then
        ...
        if [ -n "${OCP_TOOLKIT_KERBEROS:-}" ]; then
            KERBEROS=$(echo "$OCP_TOOLKIT_KERBEROS" | tr '[:upper:]' '[:lower:]' | sed 's/[^a-z0-9-]/-/g' | head -c 15)
            echo "Using kerberos for bucket prefix: ${KERBEROS}"
        elif [ -n "${OCP_TOOLKIT_BUCKET_USER:-}" ]; then
            KERBEROS=$(echo "$OCP_TOOLKIT_BUCKET_USER" | tr '[:upper:]' '[:lower:]' | sed 's/[^a-z0-9-]/-/g' | head -c 15)
            echo "Using kerberos for bucket prefix: ${KERBEROS}"
        else
            KERBEROS=$(hostname -s | tr '[:upper:]' '[:lower:]' | sed 's/[^a-z0-9-]/-/g' | head -c 15)
            echo "Using local hostname for bucket prefix: ${KERBEROS}"
        fi
        BUCKET_PREFIX="acm-${KERBEROS}-"
